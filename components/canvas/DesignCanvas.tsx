...
import { ToolbarButton } from "@/components/canvas/ToolbarButton";
import type {
	AnyCommand,
	CanvasShape,
	CanvasToolCommand,
...
export const DesignCanvas = forwardRef<DesignCanvasRef, DesignCanvasProps>(
	function DesignCanvas(
		...
		const [undoStack, setUndoStack] = useState<Array<Array<AnyCommand>>>([]);
		...
		}, [isActive, selectedId, selectedIds, shapes]);
		...
		const scale = Math.min(
			(maxWidth - containerPadding) / width,
			(maxHeight - containerPadding) / height,
			1,
		);
		...
		return (
			<div
				className={`flex flex-col ${isActive ? "ring-2 ring-violet-500" : ""}`}
				onClick={onActivate}
			>
				{/* Toolbar */}
				<div className="flex gap-1 p-2 bg-slate-100 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 rounded-t-lg">
					<ToolbarButton
						title="Select (V)"
						active={tool === "select"}
					...
					<ToolbarButton
						...
						onClick={() => {
							...
							setShapes((prev) => applyCommandsToState(prev, last, selectedId));
						}}
					>
				...
				{/* Canvas */}
				<div
					className="relative bg-slate-200 dark:bg-slate-800 rounded-b-lg overflow-hidden"
					style={{
